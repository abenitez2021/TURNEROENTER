...
import Logo from "../../assets/images/logo-color.png"; // Ajustar ruta si es necesario

export default function InformeGraficoUsuarios() {
  ...
  const buscar = async () => {
    try {
      const res = await axios.post("/informes/grafico-usuarios", {
  ...
  const exportarExcel = () => {
    const hoja = XLSX.utils.json_to_sheet(
      datos.map(({ name, value }) => ({ Usuario: name, Cantidad: value }))
    );
    const libro = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(libro, hoja, "Usuarios Atenciones");
    const buffer = XLSX.write(libro, { type: "array", bookType: "xlsx" });
    const blob = new Blob([buffer], { type: "application/octet-stream" });
    saveAs(blob, "Usuarios_Atencion.xlsx");
  };
  ...
  const exportarPDF = () => {
    ...
    const addHeader = (doc) => {
      ...
      doc.setFontSize(16);
      doc.text("INFORME DE ATENCIONES POR USUARIO", 60, 23);
    ...
    doc.save(`Usuarios_Atencion_${fechaActual}.pdf`);
  };
    ...
    <div style={{ maxWidth: "100%" }}>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <Card>
            <CardHeader title="Gráfico de Atenciones por Usuario" />
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardHeader title="Detalle de Atenciones por Usuario" />
